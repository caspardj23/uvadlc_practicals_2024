...
def main(args):
    ...
    strategy_args = {
        ...
        PGD: {
            ALPHA: args.alpha_pgd,
            EPSILON: args.epsilon_pgd,
            NUM_ITER: args.num_iter_pgd,
        },
    }
        ...
        if not args.test_crossover_defense:
            if training_strategy == STANDARD:
                for attack in [FGSM, PGD]:
                    adv_acc, adv_examples = test_attack(
                        model, testloader, attack, strategy_args[attack]
                    )
                    visualise(
                        args,
                        adv_examples,
                ...
                adv_acc, adv_examples = test_attack(
                    ...
                    testloader,
                    training_strategy,
                    strategy_args[training_strategy],
                )
                visualise(
                    args,
                    adv_examples,
...
def visualise(args, adv_examples, training_strategy, attack, save_dir=""):
    ...
    if args.visualise:
        for i, adv_ex in enumerate(adv_examples):
            ...
            ax2.axis("off")
            plt.savefig(
                output_dir + f"/train-{training_strategy}_att-{attack}_{i+1}.png"
            )
            plt.close(fig)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train and test ResNet18 on CIFAR-10 dataset"
    )
    parser.add_argument(
        "--batch_size", type=int, default=64, help="Batch size for training and testing"
    )
    parser.add_argument(
        "--valid_ratio", type=float, default=0.75, help="Validation set ratio"
    )
    parser.add_argument(
        "--augmentations",
        action="store_true",
        help="Use data augmentations for training",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--train_strats",
        nargs="+",
        choices=[STANDARD, FGSM, PGD],
        default=[STANDARD],
        help="List of loss types to use for training",
    )
    parser.add_argument(
        "--visualise", action="store_true", help="Visualise adversarial examples"
    )
    parser.add_argument(
        "--epsilon_fgsm", type=float, default=0.1, help="Epsilon for FGSM attack"
    )
    parser.add_argument(
        "--alpha_fgsm", type=float, default=0.5, help="Alpha for FGSM attack"
    )
    parser.add_argument(
        "--epsilon_pgd", type=float, default=0.01, help="Epsilon for PGD attack"
    )
    parser.add_argument(
        "--alpha_pgd", type=float, default=2, help="Alpha for PGD attack"
    )
    parser.add_argument(
        "--num_iter_pgd",
        type=int,
    ...
    parser.add_argument(
        "--test_crossover_defense", action="store_true", help="Test crossover defense"
    )
