...
def load_cifar10(batch_size=4, valid_ratio=0.75, test_bs_1=True, augmentations=False):
    ...
    transform_validtest = transforms.Compose(
        [
            transforms.ToTensor(),
            transforms.Normalize([0.4914, 0.4822, 0.4465], [0.247, 0.243, 0.261]),
        ]
    ...
    validtestset = torchvision.datasets.CIFAR10(
        root="./data", train=False, download=True, transform=transform_validtest
    )

    valid_len = int(len(validtestset) * valid_ratio)

    validset, testset = torch.utils.data.random_split(
        validtestset, [valid_len, len(validtestset) - valid_len]
    )
    ...
    testloader = torch.utils.data.DataLoader(
        testset, batch_size=1 if test_bs_1 else batch_size, shuffle=False, num_workers=2
    )
            ...
            for inputs, labels in dataloader:
                ...
                with torch.set_grad_enabled(phase == "train"):
                    ...
                    # backward + optimize only if in training phase
                        ...
                        combined_inputs = torch.cat([inputs, perturbed_data], dim=0)
                        combined_labels = torch.cat([labels, labels], dim=0)

                        outputs_combined = model(combined_inputs)
                        loss = criterion(outputs_combined, combined_labels)
